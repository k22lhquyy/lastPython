...
@login_required
def place_order():
    customer_cart = Cart.query.filter_by(customer_link=current_user.id).all()  # Thêm `.all()` để lấy danh sách
    if customer_cart:
        try:
            total = 0
            ...
            # Giả lập phản hồi từ APIService (thay thế phần tạo API call)
            create_order_response = {
                'invoice': {'state': 'pending'},  # Thay thế trạng thái bằng "pending"
                'id': 'fake-payment-id'          # ID giả để thay thế `create_order_response['id']`
            }
            ...
            db.session.commit()  # Di chuyển commit ra khỏi vòng lặp để tối ưu hóa
